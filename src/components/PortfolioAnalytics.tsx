...
const portfolioSite = process.env.NEXT_PUBLIC_PORTFOLIO_SITE_ID || 'astrokobi';
...
export function PortfolioAnalytics() {
  useEffect(() => {
    ...
    const onClick = (event: MouseEvent) => {
      ...
      if (explicit) {
        sendEvent(explicit, { ...common, merchant: anchor.dataset.merchant || url.hostname });
      } else if (anchor.rel.includes('sponsored') || anchor.dataset.affiliate === 'true') {
        sendEvent('affiliate_click', { ...common, merchant: anchor.dataset.merchant || url.hostname });
      } else if (url.origin !== window.location.origin) {
        sendEvent('outbound_click', { ...common, destination: url.hostname });
      } else {
        sendEvent('internal_recirculation', { ...common, destination: url.pathname });
      }
    };
  ...
  return (
    <>
      ...
      <Script id="portfolio-ga4" strategy="afterInteractive">
        {`window.dataLayer=window.dataLayer||[];window.gtag=function(){dataLayer.push(arguments)};gtag('js',new Date());gtag('config','${measurementId}',{portfolio_site:'${portfolioSite}',publication:'${portfolioSite}'});`}
      </Script>
    </>
  );
}
