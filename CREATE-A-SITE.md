...
- **Engine (optional)** — `llm` (the writer's OpenAI-compatible endpoint, model,
  ...
  OpenRouter or another OpenAI-compatible provider via the commented examples) and `imageProvider` (`'pexels'`
...
- **The LLM key** — required. Its name must match `llm.apiKeyEnv` in
  ...
  if you switch providers). Groq's free tier comfortably covers several sites
  on one key.
